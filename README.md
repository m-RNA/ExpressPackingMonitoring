...
- 手动下载增量包时，先完整解压，再双击包内的 `双击安装增量更新.cmd`。脚本会从用户配置中读取原 `app` 目录，安全退出正在运行的软件，校验文件后安装；失败时会尝试恢复原文件。
- 如果增量包提示安装版本过旧、应用目录尚未记录或本次必须更新启动器，请改用完整包。覆盖完整包时不要删除 `%LOCALAPPDATA%\ExpressPackingMonitoring\`，配置、数据库和录像记录会继续保留。
