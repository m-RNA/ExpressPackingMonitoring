...
- 日常使用请始终从发布包根目录的 `ExpressPackingMonitoring.exe` 启动。启动器会在后台下载经过校验的增量包，并在下次启动时自动安装。
- 手动下载增量包时，先完整解压，再双击包内的 `双击安装增量更新.cmd`。脚本会优先从用户配置中读取原 `app` 目录；无法自动定位时，可按提示把完整包根目录、`app` 目录或其中的 `ExpressPackingMonitoring.exe` 拖到窗口中。脚本会自行识别实际目录、安全退出正在运行的软件、校验文件后安装；失败时会尝试恢复原文件。
- 如果增量包提示安装版本低于补丁基线或本次必须更新启动器，请改用完整包。覆盖完整包时不要删除 `%LOCALAPPDATA%\ExpressPackingMonitoring\`，配置、数据库和录像记录会继续保留。
